...
CUGRAPH_KG_ROOT=$(cd ${THISDIR}/..;pwd)
GTEST_ARGS="--gtest_output=xml:${CUGRAPH_KG_ROOT}/test-results/"
DOWNLOAD_MODE=""
EXITCODE=0

export RAPIDS_DATASET_ROOT_DIR=${RAPIDS_DATASET_ROOT_DIR:-${CUGRAPH_KG_ROOT}/datasets}
...
if hasArg "--run-python-tests"; then
    ...
    echo "Python pytest for cuGraph-kg (single-GPU only)..."
    conda list
    cd ${CUGRAPH_KG_ROOT}/python/cugraph-kg/kg
    pytest -sv -m sg --cache-clear --junitxml=${CUGRAPH_KG_ROOT}/junit-cugraph-kg-pytests.xml --cov-config=.coveragerc --cov=cugraph-kg --cov-report=xml:${WORKSPACE}/python/cugraph-kg/cugraph-kg-coverage.xml --cov-report term --ignore=raft --benchmark-disable
    echo "Ran Python pytest for cugraph : return code was: $?, test script exit code is now: $EXITCODE"

    echo "Python benchmarks for cuGraph-kg (running as tests)..."
    cd ${CUGRAPH_KG_ROOT}/benchmarks/cugraph-kg
    pytest -sv -m sg -m "managedmem_on and poolallocator_on and tiny" --benchmark-disable
    echo "Ran Python benchmarks for cuGraph-KG (running as tests) : return code was: $?, test script exit code is now: $EXITCODE"
fi
